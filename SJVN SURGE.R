...
getSymbols("SJVN.NS", from = "2021-01-01", to = "2024-05-01")
plot(SJVN.NS$SJVN.NS.Close)
sjvn = (na.omit(SJVN.NS))

#Stationary test for closing price
close = sjvn$SJVN.NS.Close
plot(close)
adf.test(close)

#Stationary test for daily log returns 
logprice = log(close)
logreturns = periodReturn(sjvn$SJVN.NS.Close,period = 'daily', type = 'log') 
plot(logreturns)
...
summary(model2021)
Box.test(residuals(model2021), type = c("Ljung-Box")) #p-value = 0.75

getSymbols("SJVN.NS", from = "2020-01-01", to = "2024-05-01")
plot(SJVN.NS$SJVN.NS.Close)
sjvn = (na.omit(SJVN.NS))
...
summary(model2020)
Box.test(residuals(model2020), type = c("Ljung-Box")) #p-value = 0.94

arima_residuals = residuals(model2020)
#As p-value for 2020 model is higher, we consider model from 2021 data
...
# Use normalized data to create sequences
sequence_length = 12  # Window size
train_sequences <- create_sequences(normalized_train$data, sequence_length)
test_sequences <- create_sequences(normalized_test$data, sequence_length)
