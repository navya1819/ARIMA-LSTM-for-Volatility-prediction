#KPIGREEN.NS, SJVN.NS --> Green energy stocks
#EXIDEIND.NS, ARE&M.NS --> EV stocks
#KPEL.BO, BFUTILITIE.NS --> Solar stocks
...
getSymbols("^NSEI", from = "2020-01-01", to = "2024-05-01")
plot(NSEI$NSEI.Close)
nifty = (na.omit(NSEI))

#Stationary test for closing price
close = nifty$NSEI.Close
plot(close)
adf.test(close)

#Stationary test for daily log returns 
logprice = log(close)
logreturns = periodReturn(nifty$NSEI.Close,period = 'daily', type = 'log') 
plot(logreturns)
...
summary(model2020)
Box.test(residuals(model2020), type = c("Ljung-Box")) #p-value = 0.4711

getSymbols("^NSEI", from = "2021-01-01", to = "2024-05-01")
plot(NSEI$NSEI.Close)
nifty = (na.omit(NSEI))
...
summary(model2021)
Box.test(residuals(model2021), type = c("Ljung-Box")) #p-value = 0.78
arima_residuals = residuals(model2021)
#As p-value for 2021 model is higher, we consider model from 2021 data

#Using 2021 model to predict future prices
forecast2021 = forecast(model2021, length(test))

#Calculating accuracy for the model
accuracy = accuracy(forecast2021, test)
print(accuracy)
metrics = data.frame(MODEL = "ARIMA",AIC = AIC(model2021), MAE = accuracy["Test set", "MAE"], RMSE = accuracy["Test set", "RMSE"], MASE = accuracy["Test set", "MASE"])
...
qqnorm(residuals(model2021))
qqline(residuals(model2021), col = 2)
hist(residuals(model2021), breaks = 20, col = "skyblue", main = "Histogram of Residuals", xlab = "Residuals")

ArchTest(residuals(model2021)) #p-value less than 0.05 indicates ARCH effect
...
for (p_garch in 0:5) {
  for (q_garch in 0:5) {
    garch_fit <- tryCatch(garch(residuals(model2021), order = c(p_garch, q_garch), trace = FALSE), error = function(e) NULL)
    if (!is.null(garch_fit)) {
      log_likelihood <- logLik(garch_fit)
      num_params <- length(coef(garch_fit))
      num_obs <- length(residuals(model2021))
...
garch_fit <- ugarchfit(spec = garch_spec, data = residuals(model2021))  #fitting the model
...
sq_residuals = (residuals(model2021))^2
acf(sq_residuals, lag.max = 18)
#Max correlation observed for lag of 15 days

# Use normalized data to create sequences
sequence_length = 15  # Window size
train_sequences = create_sequences(normalized_train$data, sequence_length)
test_sequences = create_sequences(normalized_test$data, sequence_length)
