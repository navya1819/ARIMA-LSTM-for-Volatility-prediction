...
getSymbols("KPIGREEN.NS", from = "2021-01-01", to = "2024-05-01")
plot(KPIGREEN.NS$KPIGREEN.NS.Close)
kpigreen = (na.omit(KPIGREEN.NS))

#Stationary test for closing price
close = kpigreen$KPIGREEN.NS.Close
plot(close)
adf.test(close)

#Stationary test for daily log returns 
logprice = log(close)
logreturns = periodReturn(kpigreen$KPIGREEN.NS.Close,period = 'daily', type = 'log') 
plot(logreturns)
...
lines(results$time, results$predicted, col = "red", lwd = 2)
legend("bottomright", legend = c("Actual", "Predicted"), col = c("blue", "red"), lty = 1, lwd = 2)
