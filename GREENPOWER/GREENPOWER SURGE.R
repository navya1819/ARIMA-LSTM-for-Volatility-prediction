...
getSymbols("GREENPOWER.NS", from = "2020-01-01", to = "2024-05-01")
plot(GREENPOWER.NS$GREENPOWER.NS.Close)
greenpower = (na.omit(GREENPOWER.NS))

#Stationary test for closing price
close = greenpower$GREENPOWER.NS.Close
plot(close)
adf.test(close)

#Stationary test for daily log returns 
logprice = log(close)
logreturns = periodReturn(greenpower$GREENPOWER.NS.Close,period = 'daily', type = 'log') 
plot(logreturns)
...
summary(model2020)
Box.test(residuals(model2020), type = c("Ljung-Box")) #p-value = 0.88

getSymbols("GREENPOWER.NS", from = "2021-01-01", to = "2024-05-01")
plot(GREENPOWER.NS$GREENPOWER.NS.Close)
greenpower = (na.omit(GREENPOWER.NS))
...
summary(model2021)
Box.test(residuals(model2021), type = c("Ljung-Box")) #p-value = 0.92
...
#Maximum Lag correlation is observed at lag 14
# Normalize train and test data
normalized_train <- normalize(train)
normalized_test <- normalize(test)
