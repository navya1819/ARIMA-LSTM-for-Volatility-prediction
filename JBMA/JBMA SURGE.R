...
getSymbols("JBMA.NS", from = "2021-01-01", to = "2024-05-01")
plot(JBMA.NS$JBMA.NS.Close)
jbma = (na.omit(JBMA.NS))

#Stationary test for closing price
close = jbma$JBMA.NS.Close
plot(close)
adf.test(close)

#Stationary test for daily log returns 
logprice = log(close)
logreturns = periodReturn(jbma$JBMA.NS.Close,period = 'daily', type = 'log') 
plot(logreturns)
...
summary(model2021)
Box.test(residuals(model2021), type = c("Ljung-Box")) #p-value = 0.59

getSymbols("JBMA.NS", from = "2020-01-01", to = "2024-05-01")
plot(JBMA.NS$JBMA.NS.Close)
jbma = (na.omit(JBMA.NS))
...
summary(model2020)
Box.test(residuals(model2020), type = c("Ljung-Box")) #p-value = 0.96
